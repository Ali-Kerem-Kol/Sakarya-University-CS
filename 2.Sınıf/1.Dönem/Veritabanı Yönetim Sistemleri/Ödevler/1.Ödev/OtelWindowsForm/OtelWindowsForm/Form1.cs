    ...
    public partial class Form1 : Form
    {
        ...
        static string GeneratePersonelId(NpgsqlConnection connection, string pozisyon)
        {
            using (NpgsqlCommand command = new NpgsqlCommand())
            {
                ...
                // Personel ID'sini oluþturmak için sýralý numarayý al
        ...
        static int personelIDkontrol(string deger)
        {
            ...
            {
                // Hata durumu: Boþ veya null gelen deðer
                Console.WriteLine("Hata: Geçersiz deðer. Boþ veya null deðerler kabul edilmez.");
                return -1; // Hata durumunu temsil eden bir deðer döndürülebilir.
            }

            // Ýlk harfi kontrol et ve uygun deðeri döndür
            ...
            switch (ilkHarf)
            {
                ...
                case 'Y':
                    return 5;
                default:
                    // Hata durumu: Geçersiz baþlangýç harfi
                    Console.WriteLine($"Hata: Geçersiz baþlangýç harfi '{ilkHarf}'.");
                    return -1; // Hata durumunu temsil eden bir deðer döndürülebilir.
            }
        }
        ...
        private void btn_ekle_musteri_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Bu Müþteri ID zaten kullanýmda.", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Information);
                connection.Close();
                return;
            }
        ...
        private void btn_ekle_personel_Click(object sender, EventArgs e)
        {
            ...
            // Ayný personel_id ile kayýt olup olmadýðýný kontrol et
            if (PersonelIdVarMi(txt_personel_id.Text))
            {
                MessageBox.Show("Bu personel_id zaten kullanýmda.", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
                connection.Close();
                return;
            }
            ...
            else if (pozisyon == "Aþçý" && kontrol == 3)
            {
                personelEklemeKomutu.Parameters.AddWithValue("@p6", txt_personel_id.Text);
            }
            else if (pozisyon == "Lobi Elemaný" && kontrol == 4)
            {
                personelEklemeKomutu.Parameters.AddWithValue("@p6", txt_personel_id.Text);
            }
        ...
        private void btn_sil_personel_Click(object sender, EventArgs e)
        {
            ...
            if (!PersonelIdVarMi(txt_personel_id.Text))
            {
                MessageBox.Show("Personel Bulunamadý !", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
                connection.Close();
                return;
            }
            //---------
            ...
            try
            {
                ...
                if (kontrol == 1)
                {
                    // 1. Diðer tablolardaki kayýtlarý sil
                ...
                {
                    // 2. Diðer tablolardaki kayýtlarý sil
                    NpgsqlCommand izinSilmeKomutu = new NpgsqlCommand("DELETE FROM \"PersonelIzinleri\" WHERE personel_id=@p1", connection);
                    if (IzinVarMi(txt_personel_id.Text))
                    {
                        izinSilmeKomutu.Parameters.AddWithValue("@p1", txt_personel_id.Text);
                ...
                {
                    // 3. Diðer tablolardaki kayýtlarý sil
                    NpgsqlCommand izinSilmeKomutu = new NpgsqlCommand("DELETE FROM \"PersonelIzinleri\" WHERE personel_id=@p1", connection);
                    if (IzinVarMi(txt_personel_id.Text))
                    {
                        izinSilmeKomutu.Parameters.AddWithValue("@p1", txt_personel_id.Text);
                ...
                {
                    // 4. Diðer tablolardaki kayýtlarý sil
                    NpgsqlCommand izinSilmeKomutu = new NpgsqlCommand("DELETE FROM \"PersonelIzinleri\" WHERE personel_id=@p1", connection);
                    if (IzinVarMi(txt_personel_id.Text))
                    {
                        izinSilmeKomutu.Parameters.AddWithValue("@p1", txt_personel_id.Text);
                ...
                {
                    // 5. Diðer tablolardaki kayýtlarý sil
                    NpgsqlCommand izinSilmeKomutu = new NpgsqlCommand("DELETE FROM \"PersonelIzinleri\" WHERE personel_id=@p1", connection);
                    if (IzinVarMi(txt_personel_id.Text))
                    {
                        izinSilmeKomutu.Parameters.AddWithValue("@p1", txt_personel_id.Text);
        ...
        private void btn_sil_musteri_Click(object sender, EventArgs e)
        {
            ...
            if (!musterilIdVarMi((int)num_musteri_id.Value))
            {
                MessageBox.Show("Musteri Bulunamadý !", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
                connection.Close();
                return;
            }
        ...
        {
            Personel_Detaylarý personelDetay = new Personel_Detaylarý();

            this.Hide(); // veya this.Close(); 
            ///// ilk baþta "Detaylar..." a týklayýnca ana formu çarpý iþaretinen kapatýnca program sonlanmýyor ?!
        ...
        private void btn_ekle_oda_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Bu Oda ID zaten kullanýmda.", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Information);
                connection.Close();
                return;
            }
            if (!turIdVarMi((int)num_oda_turu.Value))
            {
                MessageBox.Show("Böyle Bir Oda Türü Mevcut Deðil !", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Information);
                connection.Close();
                return;
            }
        ...
        private void btn_sil_oda_Click(object sender, EventArgs e)
        {
            ...
            if (!odaIdVarMi((int)num_oda_id.Value))
            {
                MessageBox.Show("Oda Bulunamadý !", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
                connection.Close();
                return;
            }
            if (!turIdVarMi((int)num_oda_turu.Value))
        ...
        private void btn_ekle_rezervasyon_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Bu Rezervasyon ID zaten kullanýmda.", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Information);
                connection.Close();
                return;
            }
        ...
        private void btn_sil_rezervasyon_Click(object sender, EventArgs e)
        {
            ...
            if (!rezervasyonIdVarMi((int)num_rezervasyon_id.Value))
            {
                MessageBox.Show("Rezervasyon Bulunamadý !", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
                connection.Close();
                return;
            }
        ...
        private void btn_ekle_odeme_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Bu Odeme ID zaten kullanýmda.", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Information);
                connection.Close();
                return;
            }
        ...
        private void btn_sil_odeme_Click(object sender, EventArgs e)
        {
            ...
            if (!odemeIdVarMi((int)num_odemeid.Value))
            {
                MessageBox.Show("Ödeme ID Bulunamadý !", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Error);
                connection.Close();
                return;
            }
            if (!rezervasyonIdVarMi((int)num_odeme_rezervasyon_id.Value))
        ...
        private void btn_ekle_musteri_istek_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Bu Ýstek ID zaten kullanýmda.", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Information);
                connection.Close();
                return;
            }
            if (musterilIdVarMi((int)num_musteri_id.Value))
        ...
        private void btn_sil_musteri_istek_Click(object sender, EventArgs e)
        {
            ...
            if (!istekIdVarMi((int)num_musteri_hizmet_istekleri_istek_id.Value))
            {
                MessageBox.Show("Bu Ýstek ID Bulunamadý !!!", "Hata", MessageBoxButtons.OK, MessageBoxIcon.Information);
                connection.Close();
                return;
            }
